/*
Copyright 2015 The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package collectors

import (
	"fmt"

	"github.com/golang/glog"
	"github.com/prometheus/client_golang/prometheus"
	"k8s.io/apimachinery/pkg/util/sets"
)

// Controller defines base metrics about the service controller
type Controller struct {
	prometheus.Collector

	reconcileOperation       *prometheus.CounterVec
	reconcileOperationErrors *prometheus.CounterVec
	managedServices         *prometheus.GaugeVec

	labels prometheus.Labels
}

// NewController creates a new prometheus collector for the
// Service controller operations
func NewController(class string) *Controller {

	cm := &Controller{
		labels: prometheus.Labels{
			"class": class,
		},

		reconcileOperation: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: PrometheusNamespace,
				Name:      "success",
				Help:      `Cumulative number of Service controller reconcile operations`,
			},
			[]string{"class"},
		),
		reconcileOperationErrors: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: PrometheusNamespace,
				Name:      "errors",
				Help:      `Cumulative number of Service controller errors during reconcile operations`,
			},
			[]string{"class", "service"},
		),
		managedServices: prometheus.NewGaugeVec(
			prometheus.GaugeOpts{
				Namespace: PrometheusNamespace,
				Name:      "managed_services",
				Help:      `Total number of services managed by the controller`,
			},
			[]string{"class", "namespace"},
		),
	}

	return cm
}

// IncReconcileCount increment the reconcile counter
func (cm *Controller) IncReconcileCount() {
	cm.reconcileOperation.With(cm.labels).Inc()
}

// IncReconcileErrorCount increment the reconcile error counter
func (cm *Controller) IncReconcileErrorCount(name string) {
	l := prometheus.Labels{
		"class": cm.labels["class"],
	}
	l["service"] = name
	cm.reconcileOperationErrors.With(l).Inc()
}

// SetManagedServices sets the number of managed services
func (cm *Controller) SetManagedServices(nsmap map[string]int, registry prometheus.Gatherer) {
	l := prometheus.Labels{
		"class": cm.labels["class"],
	}

	mfs, err := registry.Gather()
	if err != nil {
		glog.Errorf("Error gathering metrics: %v", err)
		return
	}

	keep := sets.NewString()
	for k := range nsmap {
		keep.Insert(k)
	}

	for _, mf := range mfs {
		metricName := mf.GetName()
		if fmt.Sprintf("%v_managed_services", PrometheusNamespace) != metricName {
			continue
		}
		for _, m := range mf.GetMetric() {
			labels := make(map[string]string, len(m.GetLabel()))
			for _, labelPair := range m.GetLabel() {
				labels[*labelPair.Name] = *labelPair.Value
			}
			if keep.Has(labels["namespace"]) {
				continue
			}

			glog.V(2).Infof("Removing prometheus metric from gauge %v for namespace %v", metricName, labels["namespace"])
			removed := cm.managedServices.Delete(labels)
			if !removed {
				glog.V(2).Infof("metric %v for host %v with labels not removed: %v", metricName, labels["namespace"], labels)
			}
		}
	}

	for ns, cnt := range nsmap {
		c := float64(cnt)
		l["namespace"] = ns
		cm.managedServices.With(l).Set(c)
	}
}

// Describe implements prometheus.Collector
func (cm Controller) Describe(ch chan<- *prometheus.Desc) {
	cm.reconcileOperation.Describe(ch)
	cm.reconcileOperationErrors.Describe(ch)
	cm.managedServices.Describe(ch)
}

// Collect implements the prometheus.Collector interface.
func (cm Controller) Collect(ch chan<- prometheus.Metric) {
	cm.reconcileOperation.Collect(ch)
	cm.reconcileOperationErrors.Collect(ch)
	cm.managedServices.Collect(ch)
}

// RemoveMetrics removes metrics for services that have been removed
func (cm *Controller) RemoveMetrics(name string) {
	l := prometheus.Labels{
		"class": cm.labels["class"],
	}
	l["service"] = name
	cm.reconcileOperationErrors.Delete(l)
}
