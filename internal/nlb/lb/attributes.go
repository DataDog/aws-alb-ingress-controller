...
const (
	...
	CrossZoneLoadBalancingEnabledKey = "load_balancing.cross_zone.enabled"
)
...
type Attributes struct {
	...
	// CrossZoneLoadBalancing load_balancing.cross_zone.enabled - Indicates whether cross-zone load
...
func NewAttributes(attrs []*elbv2.LoadBalancerAttribute) (a *Attributes, err error) {
	a = &Attributes{
		...
		CrossZoneLoadBalancingEnabled: CrossZoneLoadBalancingEnabled,
	}
	var e error
	for _, attr := range attrs {
		...
		case CrossZoneLoadBalancingEnabledKey:
			a.CrossZoneLoadBalancingEnabled, err = strconv.ParseBool(attrValue)
			if err != nil {
				return a, fmt.Errorf("invalid load balancer attribute value %s=%s", attrKey, attrValue)
			}
		default:
...
func attributesChangeSet(current, desired *Attributes) (changeSet []*elbv2.LoadBalancerAttribute) {
	...
	if current.CrossZoneLoadBalancingEnabled != desired.CrossZoneLoadBalancingEnabled {
		changeSet = append(changeSet, lbAttribute(CrossZoneLoadBalancingEnabledKey, fmt.Sprintf("%v", desired.CrossZoneLoadBalancingEnabled)))
	}
	return
}
