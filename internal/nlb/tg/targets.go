...
type Targets struct {
	...
	// Service is the service for the targets
	Service *api.Service
...
// NewTargets returns a new Targets pointer for an Ingress
func NewTargets(targetType string, service *api.Service, backend *extensions.IngressBackend) *Targets {
	return &Targets{
...
func (c *targetsController) Reconcile(ctx context.Context, t *Targets) error {
	desired, err := c.endpointResolver.Resolve(t.Service, t.Backend, t.TargetType)
	if err != nil {
		return err
	}
